...
public class RepositoryRepresentationUtil {
    ...
    private static final Pattern REPOSITORY_WITH_TYPE_PARENTHESIS_PATTERN = Pattern.compile("^([^/]+)/([^/]+) \\(([a-z]+)\\)$");
    ...
    public static String format(RepositoryRepresentation repositoryRepresentation) {
        return String.format("%s/%s (%s)", repositoryRepresentation.namespace(), repositoryRepresentation.name(), repositoryRepresentation.type());
    }

    public static String format(Repository repository) {
        return format(new RepositoryRepresentation(repository.getNamespace(), repository.getName(), repository.getType()));
    }
