...
public class ScmPullRequestNotifier implements Notifier {

  private static final Logger LOG = LoggerFactory.getLogger(ScmPullRequestNotifier.class);

  private static final String URL = "%s/api/v2/ci/%s/%s/pullrequest/%s/%s/%s";
  ...
  ScmPullRequestNotifier(URL instance, NamespaceAndName namespaceAndName, HttpAuthentication httpAuthentication) {
